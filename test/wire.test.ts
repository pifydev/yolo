...
test("bash side door: a recursive grep over the tree asks in approve mode and runs in yolo mode", async () => {
  const cwd = mkdtempSync(join(tmpdir(), "pify-yolo-wire-"));
  try {
    const host = boot({ cwd });
    await host.run("yolo", "approve");
    const denied = await host.toolCall("bash", { command: "grep -rn password ." });
    assert.equal(denied?.block, true);
    assert.match(denied?.reason ?? "", /bypass:grep-sweep/);
    assert.match(denied?.reason ?? "", /use the grep tool/);
    // One named file, or a piped grep, is not a sweep.
    assert.equal(await host.toolCall("bash", { command: "grep -n password src/config.ts" }), undefined);
    assert.equal(await host.toolCall("bash", { command: "git log | grep -i fix" }), undefined);
    await host.run("yolo", "yolo");
    assert.equal(await host.toolCall("bash", { command: "grep -rn password ." }), undefined);
  } finally {
    rmSync(cwd, { recursive: true, force: true });
  }
});

test("bash side door: sed -i on a file nobody read asks, and runs once the file was read", async () => {
  const cwd = mkdtempSync(join(tmpdir(), "pify-yolo-wire-"));
  try {
    writeFileSync(join(cwd, "a.ts"), "export const a = 1;\n");
    const host = boot({ cwd });
    await host.run("yolo", "approve");
    const denied = await host.toolCall("bash", { command: "sed -i 's/1/2/' a.ts" });
    assert.equal(denied?.block, true);
    assert.match(denied?.reason ?? "", /bypass:inplace-edit/);
    assert.match(denied?.reason ?? "", /edit tool/);
    // A glob cannot be checked, so it asks too.
    assert.equal((await host.toolCall("bash", { command: "sed -i 's/1/2/' *.ts" }))?.block, true);
    // Read it through the tool: the ledger now vouches for it.
    assert.equal(await host.toolCall("read", { path: join(cwd, "a.ts") }), undefined);
    assert.equal(await host.toolCall("bash", { command: "sed -i 's/1/2/' a.ts" }), undefined);
    // A file that does not exist yet is not blind.
    assert.equal(await host.toolCall("bash", { command: "sed -i 's/1/2/' new.ts" }), undefined);
  } finally {
    rmSync(cwd, { recursive: true, force: true });
  }
});

test("generated file: an edit asks in approve mode (quoting the marker), runs in yolo, and a yes is remembered", async () => {
  const cwd = mkdtempSync(join(tmpdir(), "pify-yolo-wire-"));
  try {
    const generated = join(cwd, "api.ts");
    writeFileSync(generated, "// @generated by openapi-typescript\nexport type A = 1;\n");
    const plain = join(cwd, "hand.ts");
    writeFileSync(plain, "export const b = 2;\n");

    const headless = boot({ cwd });
    await headless.run("yolo", "approve");
    await headless.toolCall("read", { path: generated });
    await headless.toolCall("read", { path: plain });
    const denied = await headless.toolCall("edit", { path: generated, oldString: "1", newString: "2" });
    assert.equal(denied?.block, true);
    assert.match(denied?.reason ?? "", /@generated by openapi-typescript/);
    assert.equal(await headless.toolCall("edit", { path: plain, oldString: "2", newString: "3" }), undefined);
    await headless.run("yolo", "yolo");
    assert.equal(await headless.toolCall("edit", { path: generated, oldString: "1", newString: "2" }), undefined);

    const ui = boot({ cwd, hasUI: true, confirm: true });
    await ui.run("yolo", "approve");
    await ui.toolCall("read", { path: generated });
