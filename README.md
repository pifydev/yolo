...
The same guard now reaches the side door: `sed -i`, `perl -i` and `awk -i inplace` through `bash` rewrite a file exactly as `write` would, with no read required. When such a command names a file this session has not read (or one that changed since), or a glob that cannot be checked, it asks like a blind write and points the agent at the `edit` tool; a file it read, or one that does not exist yet, runs.

A file that says it is generated — `@generated`, `DO NOT EDIT`, `Code generated by …` in its first lines — asks too, quoting the line: the next build overwrites the change and the diff misleads everyone, so the generator or its input is the thing to edit. A yes is remembered for the session. Same gradient: `yolo` and `auto` run, `approve` and `strict` ask.
...
The withholding has a side door too: `grep -rn password .` through `bash` prints every line the `grep` tool would have hidden, and a bare `rg SECRET` is the same sweep. Such a recursive search over a tree (a directory, `.`, or no target at all) asks in `approve` and `strict`, with the advice to use the `grep` tool instead; a search of one named file, or a `| grep` on piped output, is left alone. `yolo` and `auto` run it, as they run every non-secret risk.
